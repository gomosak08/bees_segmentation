...
# === CONFIG ===
input_images = '/path/to/original/images'
input_labels = '/path/to/original/labels'
output_images = '/path/to/output/cropped_images'
output_labels = '/path/to/output/cropped_labels'
...
for file in os.listdir(input_images):
    ...
    for label in labels:
        ...
        # Filter out points outside crop (optional: remove small % outside)
        if any(x < 0 or y < 0 or x > crop_w or y > crop_h for x, y in shifted):
            continue  # skip polygons outside marco
...
print("✅ Cropped dataset prepared for segmentation training.")
