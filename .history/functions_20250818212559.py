...
def convert_to_real_area(polygon_coords, square_coords, image_width, image_height, real_square_area):
...
def load_yolo_seg_polygons(label_path):
    polys = []
    with open(label_path, "r") as f:
        for line in f:
            t = line.strip().split()
            if len(t) >= 7:  # class + 3 (x,y) pairs
