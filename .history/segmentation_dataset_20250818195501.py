...
def create_yaml(output_base: str) -> None:
    """
    Create a YOLO dataset YAML file inside the output_base directory.
    This describes the train/val/test splits, number of classes,
    class names, and roboflow metadata.
    """
    data = {
        "train": "../train/images",
        "val": "../valid/images",
    ...
    yml_path = os.path.join(output_base, "data.yaml")
    with open(yml_path, "w") as f:
        yaml.dump(data, f, sort_keys=False)

    yml_test_path = os.path.join(output_base, "test.yaml")
    with open(yml_test_path, "w") as f:
        yaml.dump(test, f, sort_keys=False)

    logging.info("Created dataset YAML: %s", yml_path)
